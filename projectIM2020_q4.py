...
if __name__ =="__main__":
    ...
    path = os.getcwd() + "\\ex4"
    images = ex4.get_database_images(path,'.jpg')
    for view in images:
        originImg = view
        img = ex4.createImage(view)
        detected_circles = ex4.createCircles(img, cv2.HOUGH_GRADIENT, 1, 20, 82, 40, 15, 40)
        # Draws those circles on the image
        # if detected_circles is not None and len(detected_circles[0]) == 1:
        ex4.drawCircle(originImg, detected_circles, (255, 0, 0), (0, 0, 255))
